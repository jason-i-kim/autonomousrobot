...
#define WHEEL_SEPARATION 0.268412 /* m 0.2662 */

//Line following definitions
#define LINE_DETECTION_LIMIT 0.4
#define reg_k 2
#define cm_array_length 2500

//IR definitions
#define OBJECT_DETECTION_RANGE 60
#define OBJECT_DETECTION_NEAR_RANGE 18
#define K_a 1645.2
#define K_b 55.4954
#define K_a_0 1458.6
#define K_b_0 70.96
...
double laserpar[10];
double MAX_SPEED;
double cm_array[cm_array_length];
int cnt, ignoreobstacles;
int line_calib_white[8] = {106, 105, 73, 82, 70, 83, 116, 71};
int line_calib_black[8] = {72, 63, 55, 60, 55, 59, 68, 56};
...
struct {double x, y, z, omega, phi, kappa, code,id,crc;} gmk;
...
void followline_wl(double speed,motiontype *m,sentype *p);
...
void update_sen(sentype *p)
{
  ...
  // IR sensor update
  //=====================================================================
  if(irsensor->data[0] > K_b_0)
    p->irarr[0] = K_a_0/(irsensor->data[0]-K_b_0);
  else
    p->irarr[0] = K_a_0/1;
    
  for(cnt = 1; cnt < 5; cnt++) {
    if(irsensor->data[cnt] > K_b)
  ...
  for(cnt = 0; cnt < 8; cnt++) {
    p->linearr[cnt] = (linesensor->data[cnt]-line_calib_black[cnt])*1.0/(line_calib_white[cnt]-line_calib_black[cnt]);
  }
...
void followline_wl(double speed,motiontype *m,sentype *p){
    ...
    if((ignoreobstacles == 0 && p->object_front_near == 0) || ignoreobstacles == 1){ 
      for(cnt = 0; cnt < 8; cnt++) {
	p->numerator += (p->linearr[cnt]) * p->dist[cnt]*500;
      	p->denominator += (p->linearr[cnt]);
      }
      for(cnt = 1; cnt < cm_array_length; cnt++) {
	cm_array[cnt] = cm_array[cnt-1];
      }
      cm_array[0] = atan(p->numerator/p->denominator/21);
      for(cnt = 0; cnt < cm_array_length; cnt++) {
	p->CM += cm_array[cnt];
      }
      p->CM /= cm_array_length;
      p->CM /= 50;
      m->motorspeed_l = speed-reg_k*p->CM;
      m->motorspeed_r = speed+reg_k*p->CM;
    }
