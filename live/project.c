	...
	switch(cmd){
	  ...
	  case cmd_wm:
	    if(sen.line_flag == 3){
	      ...
	      followline_wl(MAX_SPEED,&mot,&sen);
	    break;
	  ...
	  case cmd_eval:
	    ...
	    printf("Distance to box %f\n",-odo.y+0.22+sen.irarr[2]/100-0.13);
	    ignoreobstacles = 0;
	    state = state_read;
	    break;
	}
	//printf("Line flag %d, CM %f\n",sen.line_flag,sen.CM);
	//printf("Line sensors %d %d %d %d %d %d %d %d\n ",linesensor->data[0],linesensor->data[1],linesensor->data[2],linesensor->data[3],linesensor->data[4],linesensor->data[5],linesensor->data[6],linesensor->data[7] );
	//printf("Dist %f\n",sen.irarr[0]);
	//printf("ir left dist %d\n",irsensor->data[0]);
