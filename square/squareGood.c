...
if (p->cmd !=0){
   ...
   switch (p->curcmd){
	...
	if (sqrt(2*0.5*(p->dist-p->left_pos)) > p->speedcmd){ 
	  ...
	  p->motorspeed_l = sqrt(2*0.5*(p->dist-p->left_pos));
	}
	if (sqrt(2*0.5*(p->dist-p->right_pos)) > p->speedcmd){ 
	  ...
	  p->motorspeed_r = sqrt(2*0.5*(p->dist-p->right_pos));
	}
      }
     break;
